...
class BERTopic:
    ...
    def fit_transform(self,
        ...
        if not self.nr_topics:
            logger.logger.debug("_sort_mappings_by_frequency")
            documents = self._sort_mappings_by_frequency(documents)
            logger.logger.debug("_sort_mappings_by_frequency done")
        ...
        if documents.Document.values[0] is None:
            ...
            self._save_representative_docs(custom_documents)
             ...
             # Extract topics by calculating c-TF-IDF
            logger.logger.debug("_extract_topics")
            self._extract_topics(documents, embeddings=embeddings)
            logger.logger.debug("_extract_topics done")

            # Reduce topics
            if self.nr_topics:
                logger.logger.debug("_reduce_topics")
                documents = self._reduce_topics(documents)
                logger.logger.debug("_reduce_topics done")

            # Save the top 3 most representative documents per topic
            logger.logger.debug("_save_representative_docs")
            self._save_representative_docs(documents)
            logger.logger.debug("_save_representative_docs done")
            
        # Resulting output
        logger.logger.debug("_map_probabilities")
        self.probabilities_ = self._map_probabilities(probabilities, original_topics=True)
        logger.logger.debug("_map_probabilities done")

        logger.logger.debug("documents.Topic.to_list()")
        predictions = documents.Topic.to_list()
        logger.logger.debug("documents.Topic.to_list() done")
